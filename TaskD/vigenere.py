...
def vigenereDecryptKey(subkey: chr, subList: list) -> str:
...
def findSubKey(subList: list) -> chr:
    ...
    for subKey in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        decryptedSubMsg = vigenereDecrypt(subKey, subList)
        subKeyList.append((scoreMsg(decryptedSubMsg), subKey))

    # from subKeyList, find max score and keep that subkey. 
    max = 0
    bestSubKey = ""
    for sub in subKeyList:
        if (sub[0] > max):
            max = sub[0]
            bestSubKey = sub[1]
...
print(findKey(14, encryptedMSG))

key = "momoneyemoprobs"
# print(key.upper(), encryptedMSG)
