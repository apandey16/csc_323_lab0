...
def keyLen(input):
    ...
    # this should make the windows we are testing
    for keySize in range(1, inputLength - 1):
        window = []
        for position in range(0,inputLength, keySize):
            window.append(input[position: position + keySize])
        # print(window)
        
        #now need to score, then check curKeyLen and replace as needed
